...
A PDF/A rule is a plain `func(*Document, pdfa.Level) []pdfa.ValidationError`: it takes
...
func checkNoEncrypt(doc *Document, level pdfa.Level) []pdfa.ValidationError {
	if doc.Trailer.Get("Encrypt") != nil {
		return []pdfa.ValidationError{{
			Rule:    "6.1.3",
			Level:   level,
...
`pdfa.ValidationError` has four fields. `Rule` is the ISO 19005 clause string — what
...
through closures wrapped in `runByteCheck(level, func() []pdfa.ValidationError)`, its
...
- **Rule IDs are load-bearing.** `TestRuleCoverage` greps non-test source for
  ...
  `objNumForDict` returns 0 on a miss to match `pdfa.ValidationError.Object`, while
