...
again on `Write`. It lives in `internal/crypt`: `internal/crypt/params.go`
validates the dictionary, `internal/crypt/password.go` prepares passwords,
`internal/crypt/crypt.go` derives keys, chooses crypt filters and runs both
ciphers, and `internal/crypt/encrypt.go` builds the `/Encrypt` dictionary
`SetEncryption` installs. `crypt_api.go` is the `Document` side. Open this doc
when a file will not decrypt, when an encrypted round-trip loses data, or before
touching any of those files — the two sides must change together, since what the
builder writes is exactly what the reader re-derives and validates.
...
| Locked (wrong password, unsupported or malformed scheme) | `true` | `true` | `nil` | writes the ciphertext back unchanged under the preserved `/Encrypt` and `/ID` (the file layout is regenerated, the encrypted content is not touched), so the result still opens with the right password |
...
| `internal/saslprep` | RFC 4013 SASLprep; `internal/saslprep/tables.go` is generated by `internal/cmd/gensaslprep` |
