...
func TestParseCIDWidthsBoundsRange(t *testing.T) {
	...
	hostile, _ := parseCIDWidths(doc.view(), Array{Integer(0), Integer(2_000_000_000), Real(500)})
	if len(hostile) != 0 {
		t.Fatalf("over-wide /W range produced %d entries; must be skipped", len(hostile))
	}

	// Inverted range is skipped too.
	inverted, _ := parseCIDWidths(doc.view(), Array{Integer(100), Integer(10), Real(500)})
	if len(inverted) != 0 {
		t.Fatalf("inverted /W range produced %d entries; must be skipped", len(inverted))
	}

	// A well-formed range still expands correctly.
	valid, _ := parseCIDWidths(doc.view(), Array{Integer(10), Integer(12), Real(500)})
	...
	arr, _ := parseCIDWidths(doc.view(), Array{Integer(5), Array{Real(1), Real(2), Real(3)}})
	...
	full, _ := parseCIDWidths(doc.view(), Array{Integer(0), Integer(65535), Real(1000)})
	if len(full) != 65536 {
