...
func TestGridDefects(t *testing.T) {
	...
	if v := mustGrid(t, spanOK); len(v) != 0 {
	...
	if v := mustGrid(t, colspanOK); len(v) != 0 {
	...
	if !has(mustGrid(t, oob), "beyond the last row") {
