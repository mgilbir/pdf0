...
func TestUARealContent(t *testing.T) {
	...
	if len(checkUARealContent(artInTag.view(), cat(artInTag))) == 0 {
	...
	if len(checkUARealContent(tagInArt.view(), cat(tagInArt))) == 0 {
	...
	if len(checkUARealContent(ocWrap.view(), cat(ocWrap))) != 0 {
...
func TestUAAnnotStructType(t *testing.T) {
	...
	if len(checkUAAnnotStructType(bad.view(), bad.ResolveDict(bad.Trailer.Get("Root")))) == 0 {
		t.Error("widget under <P> not flagged")
	}
	good := mk("Form")
	if len(checkUAAnnotStructType(good.view(), good.ResolveDict(good.Trailer.Get("Root")))) != 0 {
