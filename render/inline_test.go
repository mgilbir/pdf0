...
func TestCJKBreaksBetweenIdeographs(t *testing.T) {
	pieces, _ := splitAtBreaks("日本語のテキスト", whiteSpaceOf("normal"), wordBreak{})
	if len(pieces) < 4 {
	...
	pieces, _ = splitAtBreaks("hello world", whiteSpaceOf("normal"), wordBreak{})
	var words []string
	for _, p := range pieces {
		if !p.space {
...
func TestBreakOpportunities(t *testing.T) {
	texts := func(text string) []string {
		var out []string
		pieces, _ := splitAtBreaks(text, whiteSpaceOf("normal"), wordBreak{})
		for _, p := range pieces {
			if !p.space {
