...
// Bidirectional text in layout.layout: the direction and unicode-bidi properties, and
...
func TestBidiIsNotReportedUnsupported(t *testing.T) {
	for _, text := range []string{hebrewAB, "مرحبا"} {
		got := layout.Build(layout.Input{HTML: "<p>" + text + "</p>"})
		rec := layout.NewRecorder(nil)
		w, _ := style.FromPx(1000)
		layout.Layout(got.Root, layout.Size{W: w, H: w}, nil, rec)
		for _, f := range rec.Findings() {
			if f.Rule == layout.RuleUnsupportedScript {
	...
	got := layout.Build(layout.Input{
		HTML: `<p style="direction: rtl; unicode-bidi: bidi-override">x</p>`,
	})
	for _, f := range got.Findings {
		if f.Rule == layout.RuleUnsupportedProperty &&
...
func TestRightToLeftRunIsShapedInVisualOrder(t *testing.T) {
	...
	glyphs := func(run layout.DrawText) []int {
		g, _ := face.ShapeGlyphs(layout.ShapedText(run))
	...
	ltr := glyphs(layout.DrawText{Text: "(ab"})
	rtl := glyphs(layout.DrawText{Text: "(ab", RTL: true})
