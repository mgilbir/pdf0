...
func TestUACMaps(t *testing.T) {
	...
	if v := checkOneUACMap(doc.view(), type0(Name("Identity-H"))); len(v) != 0 {
		t.Errorf("Identity-H wrongly flagged: %v", v)
	}
	// A non-predefined name is flagged.
	if v := checkOneUACMap(doc.view(), type0(Name("Adobe-Korea1-2"))); len(v) == 0 {
	...
	if v := checkOneUACMap(doc.view(), type0(badUse)); len(v) == 0 {
	...
	if v := checkOneUACMap(doc.view(), type0(goodUse)); len(v) != 0 {
	...
	if v := checkOneUACMap(doc.view(), simple); len(v) != 0 {
