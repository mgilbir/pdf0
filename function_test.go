...
func TestFuncType2(t *testing.T) {
	...
	// Linear (N=1) midpoint.
	out, ok := d.evalFunction(fn, []float64{0.5})
	...
	fn.Set("N", Real(1))
	out, ok = d.evalFunction(fn, []float64{2})
	...
	fn2.Set("N", Real(1))
	out, ok = d.evalFunction(fn2, []float64{0.3})
...
func TestFuncType3Stitching(t *testing.T) {
	...
	out, ok := d.evalFunction(fn, []float64{0.25})
	wantOut(t, out, ok, 0.5)

	// x=0.0 -> 0.
	out, ok = d.evalFunction(fn, []float64{0})
	wantOut(t, out, ok, 0)

	// x=0.75 in segment 1 -> constant 10.
	out, ok = d.evalFunction(fn, []float64{0.75})
	wantOut(t, out, ok, 10)

	// x=1.0 (top) uses the last segment.
	out, ok = d.evalFunction(fn, []float64{1})
	wantOut(t, out, ok, 10)
}
...
func TestFuncType0Sampled(t *testing.T) {
	...
	// At the grid points.
	out, ok := d.evalFunction(st, []float64{0})
	wantOut(t, out, ok, 0)
	out, ok = d.evalFunction(st, []float64{0.5})
	wantOut(t, out, ok, 128.0/255.0)
	out, ok = d.evalFunction(st, []float64{1})
	wantOut(t, out, ok, 1)

	// Between grid 0 and 1 (x=0.25 -> encoded 0.5 -> halfway 0 and 128).
	out, ok = d.evalFunction(st, []float64{0.25})
...
func TestFuncType0TwoInput(t *testing.T) {
	...
	out, ok := d.evalFunction(st, []float64{0.5, 0.7})
	wantOut(t, out, ok, 0.5)
	out, ok = d.evalFunction(st, []float64{0, 1})
	wantOut(t, out, ok, 0)
	out, ok = d.evalFunction(st, []float64{1, 0})
	wantOut(t, out, ok, 1)
}
...
func TestFuncType4Arithmetic(t *testing.T) {
	...
	// add
	out, ok := d.evalFunction(psFunc("{ 2 3 add }", dom, rng), []float64{0})
	wantOut(t, out, ok, 5)
	// sub, mul chain: (10 - 3) then unused input popped
	out, ok = d.evalFunction(psFunc("{ pop 10 3 sub }", dom, rng), []float64{0.5})
	wantOut(t, out, ok, 7)
	// input doubled: x 2 mul
	out, ok = d.evalFunction(psFunc("{ 2 mul }", dom, rng), []float64{0.4})
	wantOut(t, out, ok, 0.8)
	// neg abs sqrt
	out, ok = d.evalFunction(psFunc("{ pop -4 abs sqrt }", dom, rng), []float64{0})
	wantOut(t, out, ok, 2)
	// div and truncate
	out, ok = d.evalFunction(psFunc("{ pop 7 2 div }", dom, rng), []float64{0})
...
func TestFuncType4Transcendental(t *testing.T) {
	...
	rng := []Object{Real(-10), Real(10)}
	// sin(90 deg) = 1
	out, ok := d.evalFunction(psFunc("{ pop 90 sin }", dom, rng), []float64{0})
	wantOut(t, out, ok, 1)
	// cos(0) = 1
	out, ok = d.evalFunction(psFunc("{ pop 0 cos }", dom, rng), []float64{0})
	wantOut(t, out, ok, 1)
	// exp: 2^10 = 1024, clamped by Range [-10,10] -> 10
	out, ok = d.evalFunction(psFunc("{ pop 2 10 exp }", dom, rng), []float64{0})
	wantOut(t, out, ok, 10)
	// ln(e) = 1 (use exp to make e: 1 exp of e? use 2.718281828)
	out, ok = d.evalFunction(psFunc("{ pop 2.718281828 ln }", dom, rng), []float64{0})
	wantOut(t, out, ok, 1)
	// atan(1,1) = 45
	out, ok = d.evalFunction(psFunc("{ pop 1 1 atan }", dom, []Object{Real(0), Real(360)}), []float64{0})
	wantOut(t, out, ok, 45)
}
...
func TestFuncType4IfElse(t *testing.T) {
	...
	out, ok := d.evalFunction(psFunc(prog, dom, rng), []float64{0.8})
	wantOut(t, out, ok, 1)
	out, ok = d.evalFunction(psFunc(prog, dom, rng), []float64{0.2})
	wantOut(t, out, ok, 0.2)

	// ifelse: return 10 if x < 0.5 else 20.
	prog2 := "{ 0.5 lt { 10 } { 20 } ifelse }"
	out, ok = d.evalFunction(psFunc(prog2, dom, rng), []float64{0.1})
	wantOut(t, out, ok, 10)
	out, ok = d.evalFunction(psFunc(prog2, dom, rng), []float64{0.9})
	wantOut(t, out, ok, 20)
}
...
func TestFuncType4StackOps(t *testing.T) {
	...
	out, ok := d.evalFunction(psFunc("{ 3 1 roll }", dom, rng), []float64{0.1, 0.2, 0.3})
	...
	// keep last 3: b c a.
	out, ok = d.evalFunction(psFunc("{ 2 index exch pop exch pop }", dom, []Object{Real(0), Real(1)}), []float64{0.1, 0.2, 0.3})
	...
	out, ok = d.evalFunction(psFunc("{ pop 2 copy add }", []Object{Real(0), Real(1), Real(0), Real(1), Real(0), Real(1)}, []Object{Real(0), Real(2), Real(0), Real(2), Real(0), Real(2)}), []float64{0.1, 0.2, 0.3})
...
func TestFuncType4Comparison(t *testing.T) {
	...
	// eq
	out, ok := d.evalFunction(psFunc("{ 5 eq { 1 } { 0 } ifelse }", dom, rng), []float64{5})
	wantOut(t, out, ok, 1)
	// and (boolean)
	out, ok = d.evalFunction(psFunc("{ pop true false and { 1 } { 0 } ifelse }", dom, rng), []float64{0})
	wantOut(t, out, ok, 0)
	// not (boolean)
	out, ok = d.evalFunction(psFunc("{ pop false not { 1 } { 0 } ifelse }", dom, rng), []float64{0})
	wantOut(t, out, ok, 1)
	// bitshift: 1 << 3 = 8, clamped to Range [0,10]
	out, ok = d.evalFunction(psFunc("{ pop 1 3 bitshift }", dom, []Object{Real(0), Real(10)}), []float64{0})
	wantOut(t, out, ok, 8)
}
...
func TestFuncMalformed(t *testing.T) {
	...
	fn.Set("Domain", Array{Real(0), Real(1)})
	if _, ok := d.evalFunction(fn, []float64{0.5}); ok {
		t.Error("unknown FunctionType should fail")
	}
	// Not a dict/stream.
	if _, ok := d.evalFunction(Integer(3), []float64{0}); ok {
		t.Error("non-function object should fail")
	}
	// Type 4 with unbalanced braces.
	if _, ok := d.evalFunction(psFunc("{ 1 2 add", []Object{Real(0), Real(1)}, []Object{Real(0), Real(1)}), []float64{0}); ok {
		t.Error("unterminated program should fail")
	}
	// Type 4 with unknown operator.
	if _, ok := d.evalFunction(psFunc("{ pop bogus }", []Object{Real(0), Real(1)}, []Object{Real(0), Real(1)}), []float64{0}); ok {
		t.Error("unknown operator should fail")
	}
	// Type 4 division by zero.
	if _, ok := d.evalFunction(psFunc("{ pop 1 0 div }", []Object{Real(0), Real(1)}, []Object{Real(0), Real(1)}), []float64{0}); ok {
