...
func TestEmbedFacturXRoundTrip(t *testing.T) {
	for _, profile := range []einvoice.Profile{
		einvoice.ProfileMinimum, einvoice.ProfileBasicWL, einvoice.ProfileBasic, einvoice.ProfileEN16931, einvoice.ProfileExtended,
	} {
...
func TestEmbedFacturXUnknownProfile(t *testing.T) {
	doc := NewPDFADocument(PDFA3b)
	if err := EmbedFacturX(doc, []byte(validCII), einvoice.Profile("BOGUS"), ""); err == nil {
...
func TestFacturXXMPPacket(t *testing.T) {
	xmp := string(facturxXMPPacket(einvoice.ProfileBasic, "INVOICE", "Some & Title"))
