...
//
// # Brackets and mirrors are two properties, not one
//
...
// know which characters are brackets and which closes which: BidiBrackets.txt.
// Rule L4 then draws the character that mirrors it, which is BidiMirroring.txt.
// Neither file is derivable from the other — every bracket mirrors, and plenty
// of things that mirror are not brackets.
//
...
//
// github.com/mgilbir/forme has a sibling generator for the same four files,
// because shaping needs the same property to decide which way a run of glyphs is
// drawn. ADR 0006 records why there are two.
package main
...
func main() {
	...
	mirrors := readMirroring(filepath.Join(*ucd, "BidiMirroring.txt"))
...
// mirrorPair is a character and the one drawn in its place in a right-to-left
// run.
type mirrorPair struct{ from, to rune }

// mirrors is the Bidi_Mirroring_Glyph property, sorted by code point: rule L4.
// Only the characters that have a mirror are listed; everything else is drawn as
// it is written.
...
`)
	...
	log.Printf("wrote %s: %d class ranges, %d brackets, %d mirrors (Unicode %s)",
		*out, len(ranges), len(brackets), len(mirrors), version)
}
...
func readMirroring(path string) []mirrorPairIn {
	...
	for sc.Scan() {
		...
		out = append(out, mirrorPairIn{rune(from), rune(to)})
	}
	if err := sc.Err(); err != nil {
		log.Fatalf("reading %s: %v", path, err)
	}
	if len(out) == 0 {
		log.Fatalf("%s named no mirrored characters", path)
	}
