...
func TestUAOptionalContent(t *testing.T) {
	...
	// No /Name -> flagged.
	noName := &Dictionary{}
	if len(checkUAOptionalContent(doc.view(), mk(noName))) == 0 {
	...
	withAS.Set("AS", Array{})
	if len(checkUAOptionalContent(doc.view(), mk(withAS))) == 0 {
		t.Error("OC config with /AS not flagged")
	}
	// Proper config -> clean.
	good := &Dictionary{}
	good.Set("Name", String{Value: []byte("Default")})
	if v := checkUAOptionalContent(doc.view(), mk(good)); len(v) != 0 {
...
func TestUAEmbeddedFiles(t *testing.T) {
	...
	if len(checkUAEmbeddedFiles(mk(false, false).view())) == 0 {
		t.Error("embedded filespec missing /F and /UF not flagged")
	}
	if len(checkUAEmbeddedFiles(mk(true, false).view())) == 0 {
		t.Error("embedded filespec missing /UF not flagged")
	}
	if v := checkUAEmbeddedFiles(mk(true, true).view()); len(v) != 0 {
