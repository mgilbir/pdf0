...
func TestUALang(t *testing.T) {
	...
	if d := mk("1-pt"); len(checkUALang(d.view(), d.ResolveDict(IndirectRef{Number: 1}))) == 0 {
		t.Error("invalid catalog /Lang not flagged")
	}
	if d := mk("en-US"); len(checkUALang(d.view(), d.ResolveDict(IndirectRef{Number: 1}))) != 0 {
		t.Error("valid catalog /Lang wrongly flagged")
	}
	// Absent /Lang is not this check's concern (a separate rule requires it).
	if d := mk(""); len(checkUALang(d.view(), d.ResolveDict(IndirectRef{Number: 1}))) != 0 {
